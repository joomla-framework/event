...
`Dispatcher`, `Event`, `EventImmutable`, `AbstractEvent`, `ListenersPriorityQueue`,
`LazyServiceEventListener`, `Priority` and the interfaces have the same signatures in 3.0.0 as in
2.0.0.

The event-registry methods deprecated in 2.0 (`setEvent()`, `addEvent()`, `hasEvent()`,
`getEvent()`, `removeEvent()`, `getEvents()`, `clearEvents()`, `countEvents()`) are still present
and still emit deprecation notices — despite announcing removal in 3.0. See
